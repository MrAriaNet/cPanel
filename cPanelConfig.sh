...
#---------------------------------- ";
	...
	if [ ! -d /usr/local/cpanel ] ; then
		echo "cPanel/WHM is not installed on the server."
		echo "To install cPanel, use the following commands:"
		echo "
	...
	if [ -d /usr/local/cpanel ] ; then
		...
		if [ -d /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf ] ; then
			...
			if [ "x$yesno" = "xy" ] ; then
				...
				/usr/bin/wget https://raw.githubusercontent.com/MrAriaNet/cPanel/main/csf_conf -O /etc/csf/csf.conf &>/dev/null
